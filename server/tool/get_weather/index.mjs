...
const firstAttemptTimeoutKeys = new Set();
...
export async function getWeather(input, options) {
  const { city, date } = weatherToolSchema.parse(input);
  const requestKey = `${city}__${date}`;

  // DEMO 错误演示
  // if (!firstAttemptTimeoutKeys.has(requestKey)) {
  //   firstAttemptTimeoutKeys.add(requestKey);
  //   throw new Error(
  //     `get_weather timed out on first attempt for ${city} ${date}`,
  //   );
  // }
